...
setwd("/Users/esforsythe/Documents/Work/Career_dev/Workshops/qCMB_workshop")
...
#paste0 concatenates two strings into one long string
seqs_path<-paste0(seq_dir, seq_file_list[n])

#Read in seq file as AAstringSet object 
seqs<-readAAStringSet(filepath = seqs_path, format = "fasta")
...
aln<-msa(seqs, method = "Muscle", type = "protein", verbose = TRUE)

#Convert the alnment object to a AAbin object witha  different variable name
aln_bin<-as.AAbin(aln)

#Write a text file of the alignent
ape::write.FASTA(aln_bin, file = paste0("output_files/aln_",seq_file_list[n]))
